...
exports.prepareJS = function (jstFileName, request) {
 // if folder private/jst does not exist, create then
	...
	var jsfileExist = fs.existsSync(jsFilePath);
    var oldjstfile = false;
    if (jsfileExist) {
    // check if jst file is updated after creating js file so we need to re generate js file
    	if (fs.statSync("StudentAddForm.jst").mtime > fs.statSync(jsFilePath).ctime)
    	{
    		oldjstfile = true;
    	}
    }
	
    if (!jsfileExist || oldjstfile) {
