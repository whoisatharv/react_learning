...
if (w[1].toUpperCase().endsWith(".JST")) {
	...
	while(e < mapping.paths.length) {
		if(mapping.paths[e].path == w[1]&& mappings.paths[e].resource) {
			request.resource = mapping.paths[e].resource;
			request.isClientSideTechnologyResource = false;
			return request;
		}
		if (mappings.path[e].module && (w[1].startsWith(mapping.paths[e].path+ "/")||mappings.path[e].path )) {
			if (mapping.paths[e].methods)
				...
				if(mappings.paths[e].methods[methodKey]){
					if(mappings.paths[e].module) {
						...
						request.serviceMethod = mapping.paths[e].methods[methodKey];
						return request;
					}
